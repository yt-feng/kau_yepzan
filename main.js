...
const PRODUCT_PAGE_KEYS = ['bundle', 'chinese101', 'practice'];
...
const getLang = () => {
  let saved;
  try { saved = localStorage.getItem('lang'); } catch { /* Browser language is the fallback. */ }
  return SUPPORTED_LANGS.includes(saved) ? saved : detectBrowserLang();
};
const currentLang = getLang();
const baseCopy = COPY[currentLang] || COPY.en;
const t = { ...(currentLang === 'ar' ? { ...baseCopy, ...AR_OVERRIDES } : baseCopy), ...(window.EXPERIENCE_COPY?.[currentLang] || {}) };
const STATIC_COPY = {
...
const startLink = $('navCheckout');
if (startLink) startLink.href = 'learn.html';
const policyLabels = {en:['Terms','Refunds','Privacy'],zh:['使用条款','退款政策','隐私政策'],ar:['الشروط','الاسترداد','الخصوصية']}[currentLang];
document.querySelectorAll('footer a').forEach(link => {
  const index = ['terms.html','refund.html','privacy.html'].indexOf(link.getAttribute('href'));
  if (index >= 0) link.textContent = policyLabels[index];
});
...
document.querySelectorAll('.lang-btn').forEach((button) => {
  ...
  button.addEventListener('click', () => { try { localStorage.setItem('lang', button.dataset.lang); } catch { /* Keep browsing when storage is disabled. */ } window.location.reload(); });
});

function renderHome() { window.renderDiscoveryHome?.(currentLang); }
...
function renderLearn() { window.startLearning?.(currentLang); }
